...
struct StreetPass_MainView: View {
    ...
    var body: some View {
        NavigationView {
            List {
                Section {
                    MyEncounterCardView(card: viewModel.myCurrentCard) // Updated to show drawing
                    
                    Button(viewModel.isEditingMyCard ? "Manage My Card" : "Edit My Card") {
                        if viewModel.isEditingMyCard {
                            // If already editing, this button doesn't need to do much
                            // as save/cancel are separate. Could perhaps dismiss editor if needed.
                        } else {
            ...
            .listStyle(.grouped)
            .toolbar { // CORRECTED: Simplified toolbar structure
                ToolbarItemGroup(placement: .navigationBarLeading) { // Use ToolbarItemGroup for clarity
                    Image(systemName: "wave.3.left.and.right.circle.fill")
                        .font(.title2)
                        .foregroundColor(AppTheme.primaryColor)
                }
                // Example for trailing item if needed later:
        ...
        .navigationViewStyle(.stack) // Use stack style for typical iOS navigation
        .sheet(isPresented: $viewModel.isDrawingSheetPresented) {
            // Ensure DrawingEditorSheetView is accessible (defined in DrawingCanvasView.swift and target member)
...
struct StreetPassControlsView: View { /* ... Copy from previous 'Corrected' response ... */
...
struct StatusIndicatorView: View { /* ... Copy from previous 'Corrected' response ... */
    let label: String; let isOn: Bool
    var body: some View {
        HStack(spacing: 4) {
...
// MARK: - MessageView
struct MessageView: View { /* ... Copy from previous 'Corrected' response ... */
    let message: String; enum MessageType { case info, error, warning }; let type: MessageType
...
// MARK: - Card Display Views (MyEncounterCardView, ReceivedEncounterCardRowView)
struct MyEncounterCardView: View { /* ... Copy from previous 'Corrected' response ... */
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            if let drawingUiImage = card.drawingImage {
                ...
                HStack { Spacer(); Text("No Drawing Set For This Card").font(.caption).foregroundColor(.secondary).padding(); Spacer() }
                .frame(maxWidth: .infinity).frame(height: drawingDisplayMaxHeight / 2.5)
            ...
            if let t1 = card.flairField1Title, let v1 = card.flairField1Value, !t1.isEmpty || !v1.isEmpty { FlairDisplayRow(title: t1, value: v1) }
            if let t2 = card.flairField2Title, let v2 = card.flairField2Value, !t2.isEmpty || !v2.isEmpty { FlairDisplayRow(title: t2, value: v2) }
...
struct FlairDisplayRow: View { /* ... Copy from previous 'Corrected' response ... */
...
struct ReceivedEncounterCardRowView: View { /* ... Copy from previous 'Corrected' response ... */
    let card: EncounterCard
...
struct EncounterCardEditorView: View {
    ...
    private let avatarOptions: [String] = [ "person.fill", /* ... many more ... */ "studentdesk" ] // Keep your full list

    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            VStack(alignment: .leading) {
                Text("Card Drawing").font(.callout).fontWeight(.medium) // Updated font
                if let img = card.drawingImage {
                    Image(uiImage: img).resizable().scaledToFit()
                        .frame(maxWidth: .infinity, maxHeight: 120) // Increased preview height
                ...
                HStack {
                    ...
                    if card.drawingData != nil { Spacer(); Button(action: onRemoveDrawing) { Label("Remove", systemImage: "xmark.circle") }.buttonStyle(.bordered).tint(AppTheme.negativeColor) } // Shortened label
                }
            }
            Divider()
            Text("Text Details").font(.callout).fontWeight(.medium) // Updated font
            ...
            FlairEditorSection(
                titleBinding1: titleBinding(for: \.flairField1Title), valueBinding1: valueBinding(for: \.flairField1Value),
                titleBinding2: titleBinding(for: \.flairField2Title), valueBinding2: valueBinding(for: \.flairField2Value)
            )
        }
        .textFieldStyle(.roundedBorder)
    }

    // CORRECTED: FlairEditorSection and helper bindings are defined only ONCE here.
    struct FlairEditorSection: View {
        @Binding var titleBinding1: String; @Binding var valueBinding1: String
        @Binding var titleBinding2: String; @Binding var valueBinding2: String
        var body: some View {
...
struct StreetPass_MainView_Previews: PreviewProvider {
    static var previews: some View {
        ...
        // CORRECTED: 'attrs' defined in a scope accessible by subsequent drawing blocks
...
// Helper for String trimming in ReceivedEncounterCardRowView
extension String {
    var trimming: String {
